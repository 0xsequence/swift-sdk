...
| Wallet signing, transactions, import, and owner access methods | SDK-local validation or fee-selection failure | `.validationError` | Correct parameters or local fee selection; do not retry as an upstream outage | Absent | `PublicErrorContractsTests.swift`, `WalletImportTests.swift`, `MockWalletTests.swift` |
| `omsWallet.wallet.getWalletImportRecipientKey`, `importWallet`, `importEncryptedWallet` | Recipient-key attestation is missing, stale, malformed, untrusted, or does not match the request/response | `.attestationVerificationFailed` with the wallet-import operation | Do not encrypt or submit key material; retry only after confirming the configured PCR0 allowlist and WaaS environment | Absent | `WalletImportTests.swift` |
